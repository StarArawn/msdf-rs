use crate::{signed_distance::SignedDistance, vector::Vector2, EdgeColor};
...
pub fn signed_distance(
    ...
    let mut param = -Vector2::dot_product(qa, ep_dir) / Vector2::dot_product(ep_dir, ep_dir);
    {
        ...
        let distance = non_zero_sign(Vector2::cross_product(ep_dir, p3 - origin)) as f64
            * (p3 - origin).length();
        if fabs(distance) < fabs(min_distance) {
            min_distance = distance;
            param = Vector2::dot_product(origin + ep_dir - p3, ep_dir)
    ...
    for i in 0..MSDFGEN_CUBIC_SEARCH_STARTS {
        ...
        for step in 0..MSDFGEN_CUBIC_SEARCH_STEPS {
